...
<head>
    ...
    <title>La liste des employés</title>
                                ...
                                <table class="table align-items-center table-flush">
                                    ...
                                    <c:forEach items="${users}" var="user">
                                        <tr>
                                            ...
                                            <td>${user.email}</td>
                                            <td>
                                                <fmt:formatNumber value="${user.soldeConge}" maxFractionDigits="2"/>
                                                jours
                                            </td>
                                            <td>${user.salaire} DH</td>
                                            <td>${user.montant} DH</td>
                                            <td>
                                                <c:if test="${action == 'verser'}">
                                                    <a href="/users/verser/${user.id}"
                                                       class="btn btn-sm btn-success">Verser</a>
                                                </c:if>
                                                <c:if test="${action == 'incrementerConge'}">
                                                    <a href="/users/incrementerConge/${user.id}"
                                                       class="btn btn-sm btn-success">Incrementer congé</a>
                                                </c:if>
                                            </td>
                                        </tr>
                                    </c:forEach>
